...
save('C:\Users\k.kaboutari\Desktop\13by13 MS by Abdel\Data\BeamCombinations.mat', 'BeamCombinations');
...
for dataSetCounter = 1 : 1 : noDesData
    ...
    for angleCounter = 1 : 1 : angleSamplingNo
        ...
        % Pattern = @(u, v) sum(amp * exp(j * phase) * sinc(sx * (u - u0) / (2 * pi)) .* sinc(sy * (v - v0) / (2 * pi)));
