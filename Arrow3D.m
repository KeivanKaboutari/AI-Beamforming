...
%
%   Arrow3D(P1,P2,S,W,H,IP,ALPHA,BETA) can be used to specify properties
...
%                          Color Equivalencies
%     -------------------------------------------------------------
%     ColorOrder     Arrow3D        |     Simulink       Arrow3D
%     ----------     ----------     |     ----------     ----------
...
%
%   Arrow3D(P1,P2,S,W,H,'cone',...) will plot cones with bases centered
...
%   option.
%
%   HN = Arrow3D(P1,P2,...) returns a vector of handles to line and
%   surface objects created by Arrow3D.
%
%   Arrow3D COLORS will plot a table of named colors with default
%   brightness.  Arrow3D('colors',BETA) will plot a table of named
%   colors with brightness BETA.
%
%   Arrow3D attempts to preserve the appearance of existing axes.  In
%   particular, Arrow3D will not change XYZLim, View, or CameraViewAngle.
%   Arrow3D does not, however, support stretch-to-fill scaling.  AXIS
...
%   DASPECT, PBASPECT, AXIS, or XYZLIM commands before calling Arrow3D.
%   Changing limits or aspect ratios after calling Arrow3D may alter the
%   appearance of arrowheads and initial point markers.  Arrow3D sets
...
%   manual for 3D plots.
%
%   Arrow3D UPDATE will restore the appearance of arrowheads and
%   initial point markers that have become corrupted by changes to
%   limits or aspect ratios.  Arrow3D('update',SF) will redraw initial
...
%
%   Arrow3D UPDATE COLORS will update the magnitude coloring of
%   arrowheads, initial point markers, and lines to conform to the
%   current ColorMap.
%
%   HN = Arrow3D('update',...) returns a vector of handles to updated
%   objects.
%
%   EXAMPLES:
%
...
%     Arrow3D([0 0],[1 3])
%     Arrow3D([0 0],[1 2],'-.e')
%     Arrow3D([0 0],[10 10],'--x2',1)
%     Arrow3D(zeros(10,2),50*rand(10,2),'x',1,3)
%     Arrow3D(zeros(10,2),[10*rand(10,1),500*rand(10,1)],'u')
%     Arrow3D(10*rand(10,2),50*rand(10,2),'x',1,[],1)
%
%     % 3D vectors
%     Arrow3D([0 0 0],[1 1 1])
%     Arrow3D(zeros(20,3),50*rand(20,3),'--x1.5',2)
%     Arrow3D(zeros(100,3),50*rand(100,3),'x',1,3)
%     Arrow3D(zeros(10,3),[10*rand(10,1),500*rand(10,1),50*rand(10,1)],'a')
%     Arrow3D(10*rand(10,3),50*rand(10,3),'x',[],[],0)
%
%     % 3D animation
...
%     hold on
%     pbaspect([1.8,1.4,1])
%     Arrow3D(zeros(3),diag([500,1.5,1.5]),'l',0.7,[],0)
%     p=[20*t,u,v]; inc=4:1:length(t);
%     p2=p(inc,:); p1=p(inc-1,:);
%     hn=Arrow3D(p1(1,:),p2(1,:),'0_b',0.7);
%     for i=2:1:length(p1)
%       delete(hn)
%       hn=Arrow3D(p1(i,:),p2(i,:),'0_b',0.7);
%       pause(0.01)
%     end
%     hold off
%
...
%     Arrow3D(p1,p2,'x',2,4,'cone'), hold on
%     plot3(p1(:,1),p1(:,2),p1(:,3)), hold off
%     pause % change cone size
%     Arrow3D('update',[1,2])
%
%     % Just for fun
%     Arrow3D(zeros(100,3),50*rand(100,3),'x',8,4,[],0.95)
...
%     theta=[0:pi/22:pi/2]';
%     Arrow3D(zeros(12,2),[cos(theta),sin(theta)],'1.5o',1.5,[],[],[],0.5)
%
...
%     w=[1,2,3,4]; h=[4,6,4,2];
%     Arrow3D(zeros(4,2),[10*rand(4,1),500*rand(4,1)],'o*/',w,h,0)
%
%     % Magnitude coloring
%     colormap spring
%     Arrow3D(20*randn(20,3),50*randn(20,3),'|',[],[],0)
%     set(gca,'color',0.7*[1,1,1])
%     set(gcf,'color',0.5*[1,1,1]), grid on, colorbar
%     pause % change the ColorMap and update colors
%     colormap hot
%     Arrow3D('update','colors')
%
%     % LogLog plot
...
%     Arrow3D(p1,p2,'o'), grid on, hold off
%
%     % SemiLogX plot
...
%     q1=[1e7,1e6,1e5,1e4,1e3,1e7,1e7,1e7,1e7,1e7,1e7,1e6,1e5,1e4,1e3];
%     q2=1e-2*[9,9,9,9,9,7,5,4,3,2,1,1,1,1,1]; p2=[q1',q2'];
%     Arrow3D(p1,p2,'x'), grid on, hold off
%
%     % SemiLogY plot
...
%     set(gca,'LineStyleOrder',{'-','--','-.',':'})
%     Arrow3D(p1,p2,'*',1,[],0), grid on, hold off
%
%     % Color tables
%     Arrow3D('colors')           % default color table
%     Arrow3D('colors',0.3)       % low contrast color table
%     Arrow3D('colors',0.5)       % high contrast color table
%
%     % Update initial point markers and arrowheads
%     % relative to the current PlotBox diagonal
%     Arrow3D('update')           % redraw same size
%     Arrow3D('update',2)         % redraw double size
%     Arrow3D('update',0.5)       % redraw half size
%     Arrow3D('update',[0.5,2,1]) % redraw W half size,
%                                %        H double size, and
%                                %        IP same size
%
...
fig=gcf; ax=gca;
if strcmpi(p1,'update'), ad=getappdata(ax,'Arrow3D');    % update
  LocalLogCheck(ax);
  if size(ad,2)<13, error('Invalid AppData'), end
  setappdata(ax,'Arrow3D',[]); sf=[1,1,1]; flag=0;
  if nargin>1
...
if nargin>5 && ~isempty(ip) && strcmpi(ip,'cone')  % cone plot
  ...
  set(ax,'tag','Arrow3DConePlot');
elseif ~any(p), error('P1 cannot equal P2')
end
if ~all(p)
  warning('Arrow3D:ZeroMagnitude','Zero magnitude ignored')
...
if strcmp(get(ax,'nextplot'),'add') && strcmp(get(fig,'nextplot'),'add')
  ...
  if xys, mode='auto';
    ...
    if strcmp(get(ax,'WarpToFill'),'on')
      warning('Arrow3D:WarpToFill',['Stretch-to-fill scaling not ',...
        'supported;\nuse DASPECT or PBASPECT before calling Arrow3D.']);
    end
  end
...
if nargin<3, [c,ls,lw]=LocalValidateCLSW;% default color, linestyle/width
  ...
  elseif c=='o'                                    % ColorOrder
    if ~isempty(ColorOrder)
      ...
      if failed, ColorOrderWarning=['Invalid ColorOrder ',...
        ...
        warning('Arrow3D:ColorOrder',ColorOrderWarning)
      else, C=c;
      end
    end, c=C;
  ...
  elseif ~sum(vc==c), c='k'; ColorWarning=['Invalid color switch, ',...
    ...
    warning('Arrow3D:Color',ColorWarning)
  end
end
...
if lw=='/'                                         % LineWidthOrder
  if ~isempty(LineWidthOrder)
    ...
    warning('Arrow3D:LineWidthOrder',LineWidthOrderWarning)
  end
end
...
%-------------------------------------------------------------------------
% Line
set(ax,'DefaultLineTag','Arrow3D');
if ~cone
  H1=Zeros;
  for i=1:n
    if lw(i)
      H1(i)=plot3([p1(i,1),p2(i,1)],[p1(i,2),p2(i,2)], [p1(i,3),p2(i,3)],ls{i},'Color',c(i,:),'LineWidth', lw(i));
    end
  end
else                                               % cone plot
  P=zeros(3*n,3); i=1:n;
...
setappdata(ax,'Arrow3D',[getappdata(ax,'Arrow3D');p1,p2,c,w,h,ip,a]);
...
set(ax,'DefaultSurfaceTag','Arrow3D','DefaultSurfaceEdgeColor','none');
...
delete(H(strcmp(tag,'Arrow3D') & strcmp(type,'surface')));
set(fig,'nextplot','add'); set(ax,'nextplot','add'); H1=[];
if flag, map=get(fig,'colormap');                  % update colors
  M=(p1-p2); M=sqrt(sum(M.*M,2)); minM=min(M); maxM=max(M);
  H1=H(strcmp(tag,'Arrow3D') & strcmp(type,'line'));
  ...
  if length(H1)>1
    for i=1:length(H1)                             % update line colors
      ...
      if length(x)>2                               % multiple lines
        warning('Arrow3D:Magnitude',MagnitudeWarning), continue
      end
      m=sqrt((x(1)-x(2))^2+(y(1)-y(2))^2+(z(1)-z(2))^2);
      c=LocalInterp(minM,maxM,map,m); set(H1(i),'color',c);
    end
  elseif length(H1)==1
    warning('Arrow3D:Magnitude',MagnitudeWarning)
  end
  c=LocalInterp(minM,maxM,map,M);
end
set(ax,'ColorOrder',c);                            % update surfaces
ColorOrder=[];
if strcmp(get(ax,'tag'),'Arrow3DConePlot')
      H=Arrow3D(p1,p2,'o' ,w,h,'cone',a);           % update cones
else, H=Arrow3D(p1,p2,'o0',w,h,    ip,a);
end,  H=[H1(:);H(:)];
set(ax,'nextplot','replace');
...
if n, clf reset                                    % plot color table
  ...
  for i=1:24, subplot(4,6,i); box on
    ...
    if rem(i,6)==1
      ...
      if i==19
        text(0,-0.25,['{\bf\itArrow3D}  Named Color Table  ',...
            '( \beta = ',num2str(beta),' )']);
      end
    end
  end
